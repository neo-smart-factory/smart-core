...
const { TonClient, WalletContractV5R1, internal, beginCell, Address, toNano } = require('@ton/ton');
...
async function executeTONMint(config) {
    ...
    const endpoint = isTestnet
        ? (process.env.TON_RPC_URL_ONFINALITY_TESTNET || 'https://testnet.toncenter.com/api/v2/jsonRPC')
        : (process.env.TON_RPC_URL_ONFINALITY_MAINNET || 'https://toncenter.com/api/v2/jsonRPC');
    ...
    const contentCell = beginCell().storeUint(0x01, 8).storeStringTail(contentUri).endCell();

    // 5. Build Deploy Message
    const queryId = Math.floor(Date.now() / 1000);
    const ownerAddress = Address.parse(config.ownerAddress || wallet.address.toString());
        ...
        req.on('end', async () => {
            try {
                ...
                if (!signature || !validateSignature(payload, signature)) {
                    ...
                    return res.end(JSON.stringify({ error: 'Unauthorized: Invalid Genesis Signature' }));
                }
                ...
                if (payload.event === 'MINT_REQUESTED') {
                    ...
                    res.end(JSON.stringify({ status: 'processing', message: 'Mint order accepted' }));
