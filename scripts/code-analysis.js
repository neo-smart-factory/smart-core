...
const rules = [
    ...
    check(lines, filePath) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (/^['"`]$/.test(line)) {
          issues.push({
            line: i + 1,
            message: `Stray character '${line}' found on its own line`,
          });
        }
      }
      return issues;
...

  {
    ...
    severity: "error",
    description:
      "Named return parameters shadowing inherited functions (name, symbol, etc.)",
    check(lines, filePath) {
      const issues = [];
        ...
        if (inReturns) {
          ...
          for (const kw of shadowKeywords) {
            ...
            if (
              ...
              issues.push({
                line: i + 1,
                message: `Return parameter '${kw}' may shadow inherited function. Rename to e.g. 'token${
...

  {
    ...
    severity: "error",
    description:
      "Email-like addresses in NatSpec comments using @ (parsed as tags)",
    check(lines, filePath) {
      const issues = [];
      let inComment = false;
      for (let i = 0; i < lines.length; i++) {
        ...
        if (inComment || line.startsWith("*") || line.startsWith("//")) {
          ...
          for (const match of atMatches) {
            if (
              !validTags.some((tag) => match.startsWith(tag.replace("@", "@")))
            ) {
              // Check if it looks like email (has dots after @)
              const fullMatch = line.match(
                new RegExp(`${match.replace("@", "@")}[\\w.]*`)
              );
...

  {
    ...
    check(lines, filePath) {
      const issues = [];
      const hasPragma = lines.some((l) =>
        l.trim().startsWith("pragma solidity")
      );
      if (!hasPragma) {
        issues.push({
          line: 1,
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (
          ...
          if (
            ...
            line.includes(".transfer(address(this).balance)")
          ) {
            issues.push({
              line: i + 1,
              message:
                'Using .transfer() for ETH. Consider .call{value: }("") for forward-compatibility with gas changes',
            });
          }
        }
      }
...
function analyze() {
  ...
  console.log(
    `${COLORS.dim}  Scanning ${files.length} Solidity files...${COLORS.reset}\n`
  );
  ...
  if (fileIssues.length === 0) {
    console.log(
      `  ${COLORS.green}✅ All ${files.length} contracts passed analysis. Ready to compile!${COLORS.reset}\n`
    );
    return 0;
  }
  ...
  for (const { file, issues } of fileIssues) {
    ...
    for (const issue of issues) {
      ...
      console.log(
        `    ${severity} L${issue.line}: [${issue.ruleId}] ${issue.message}`
      );
    }
    console.log("");
  }
  ...
  console.log(
    `  ${COLORS.bold}Summary:${COLORS.reset} ${COLORS.red}${totalErrors} errors${COLORS.reset}, ${COLORS.yellow}${totalWarnings} warnings${COLORS.reset}`
  );
  console.log(
    `  ${COLORS.dim}Run 'npx hardhat compile' only after resolving all errors.${COLORS.reset}\n`
  );
