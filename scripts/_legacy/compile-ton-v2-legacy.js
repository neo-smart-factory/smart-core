...
const targets = [
    'NeoJettonMinter.fc',
    'NeoJettonWallet.fc',
    'NeoJettonFactoryV2.fc'  // Factory V2
];

async function main() {
    console.log('🚀 Compiling TON Contracts (Factory V2)...\n');
        ...
        try {
            const result = await compileFunc({
                ...
                sources: (p) => {
                    ...
                    if (!content && !p.endsWith('stdlib.fc')) {
                        console.error(`\nCould not find: ${p}`);
                    }
                    return content || '';
                }
    ...
    if (hasErrors) {
        ...
        console.log('\n📦 Built contracts:');
        console.log('   - NeoJettonFactoryV2 (Factory V2)');
        console.log('   - NeoJettonMinter');
        console.log('   - NeoJettonWallet');
    }
}
